...
export default async function WorksPage({ searchParams }: { searchParams: WorksSearchParams }) {
  ...
  const sort = (firstParam(params.sort) as WorkFilterInput["sort"]) ?? "year-desc";
  const page = Math.max(1, Number(firstParam(params.page) ?? "1") || 1);
  const filteredWorks = filterWorks({ query, category, subcategory, difficulty, region, movement, beginner, sort });
  ...
  function href(overrides: Record<string, string | number | undefined>) {
    ...
    for (const [key, value] of Object.entries({ q: query, category, subcategory, difficulty, region, movement, beginner, sort, page: currentPage, ...overrides })) {
      ...
      <section className="work-index-layout" aria-label="作品地图索引">
        <aside className="work-filter-panel" aria-label="作品筛选">
          ...
          <details>
            ...
            <div className="filter-link-group">
              <Link className={sort === "year-desc" ? "filter-link active" : "filter-link"} href={href({ sort: "year-desc", page: 1 })}>最新出版 ↓</Link>
              <Link className={sort === "year-asc" ? "filter-link active" : "filter-link"} href={href({ sort: "year-asc", page: 1 })}>最早出版 ↑</Link>
              <Link className={sort === "difficulty" ? "filter-link active" : "filter-link"} href={href({ sort: "difficulty", page: 1 })}>难度从低到高</Link>
              <Link className={sort === "title" ? "filter-link active" : "filter-link"} href={href({ sort: "title", page: 1 })}>按名称排序</Link>
            </div>
          </details>
        ...
        <div className="work-results">
          ...
          <section className="card-grid work-grid" aria-label="作品列表">
            {visibleWorks.map((work) => <WorkCard work={work as any} key={work.id} />)}
          </section>
