...
import { getMovement, movements, readingPaths } from "@/lib/content";
...
function findMovementPath(movement: ReturnType<typeof getMovement>) {
...
export default async function MovementDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const path = findMovementPath(movement);
    ...
    <main className="page detail-stack">
      <section className="hero movement-detail-hero">
        ...
        <aside className="decision-panel" aria-label="第一本建议">
          ...
          <a className="text-link" href="#reading-path">查看为什么排第一</a>
        </aside>
      </section>
      ...
      <section className="detail-section">
        ...
        <dl className="meta-list">
          ...
          <div><dt>代表作品数量</dt><dd>{movement.representativeWorksCount}</dd></div>
          <div><dt>是否适合新手</dt><dd>{movement.beginnerFriendly}</dd></div>
          <div><dt>内容状态</dt><dd>{movement.aiGenerated ? "AI 草稿" : "人工内容"} / {movement.reviewStatus}</dd></div>
        </dl>
      </section>
      ...
      <section className="section-grid movement-explain-grid">
        ...
        <article className="field-card"><strong>新手阅读提醒</strong><p>{movement.beginnerWarning}</p></article>
                ...
                <div className="movement-work-links">
                  {cards.length ? cards.map((card) => (
                    <Link href={`/works/${card.workId}`} key={card.id}>
                      ...
                      <span>难度 {card.difficultyLevel} / {card.caution ? "建议暂缓" : "可按路径进入"}</span>
                    </Link>
